...
export default function CreateTaskPage() {
  ...
  // Fetch available days when batch is selected
  useEffect(() => {
    const fetchAvailableDays = async () => {
      ...
      try {
        ...
        // Update form data with selected batch ID
        setFormData(prev => ({
          ...prev,
          batchId: selectedBatch,
          dayNumber: data.availableDays.length > 0 ? data.availableDays[0] : ''
        }));
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect to task list or view page
      setTimeout(() => {
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              <label className="flex items-center">
                ...
                />
                <span className="ml-2 text-sm text-gray-700">Publish task immediately</span>
              </label>
              <p className="mt-1 text-xs text-gray-500">
                If unchecked, the task will be saved as a draft.
              </p>
            </div>
          </div>
